  ...
  if (email != "") {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const isPC = window.innerWidth > 1024; // Assume width > 1024px is a PC
        ...
        if (isPC) {
          ...
          profileIn.style.display = 'block';
        }

        // Update the display logic on window resize
        window.onresize = () => {
...
async function regi() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      alert("Register for event success");
